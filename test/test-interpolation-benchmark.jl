...
@testset "satellite_wet_blend acts only where the satellite reports rain" begin
    ...
    @test satellite_wet_blend_prediction(y_sat, prediction, fallback, 1.0, 0.1) ==
        [1.0 20.0; 30.0 4.0]
    # The test is `>=`: a value exactly at the threshold is wet.
...
@testset "blended methods and derived products are opt-in and never define the shared mask" begin
    # Off by default, so a run written before the option existed reports exactly what it did.
    @test benchmark_methods((; satellite_wet_blend=false, blend_axes=[:constant])) ==
        BENCHMARK_METHODS
    enabled = benchmark_methods((; satellite_wet_blend=true, blend_axes=[:constant]))
    ...
    # A second axis reports a second blended method beside the constant one rather than replacing
    # it, so the two weight rules are comparable on identical cells.
    banded = benchmark_methods((;
        satellite_wet_blend=true, blend_axes=[:constant, :agreement_envelope]))
    @test banded[(length(BENCHMARK_METHODS) + 1):end] == [
        "blend_residual_gwr", "blend_mixed_gwr", "blend_mgwr",
    ...
    # `MASK_METHODS` would silently re-score all the others.
    for method in banded[(length(BENCHMARK_METHODS) + 1):end]
    ...
    # Derived products are appended after the ones with files, in a fixed order.
    products = ["FY4B", "GPM", "GSMaP"]
    @test benchmark_products((; fused_anchor_variants=Symbol[]), products) == products
    @test benchmark_products((; fused_anchor_variants=[:ols, :mean]), products) ==
...
@testset "Benchmark runs end to end on the inner spatial split" begin
    mktempdir() do temp_dir
        ...
        # cannot make: that the weight is chosen per fold on the inner split, that the blended
        # methods are reported without disturbing anything else, and that turning the option on
        # leaves every pre-existing method's numbers exactly as they were.
